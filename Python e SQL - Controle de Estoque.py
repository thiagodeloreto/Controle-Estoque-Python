...
def deletar_insumo():
    if len(nome_insumo.get()) < 2:
        caixa_texto.delete("1.0", END)
        caixa_texto.insert("1.0", f"{nome_insumo.get()} -> Nome do insumo inválido!")
        return
...
def consumir_insumo():
   if len(nome_insumo.get()) < 2 or len(lote_insumo.get()) < 1 or len(qtde_insumo.get()) < 1:
        caixa_texto.delete("1.0", END)
        caixa_texto.insert("1.0", f"{nome_insumo.get()} -> Nome, Quantidade e Lote do insumo inválido!")
        return
...
def visualizar_insumo():
    ...
    if not valores:  
        caixa_texto.delete("1.0", END)
        caixa_texto.insert("1.0", f"Produto '{nome_insumo.get()}' não encontrado no banco de dados.")
        return
